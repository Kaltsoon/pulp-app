var UI = (function(){
	...
	var display_mean_and_variance = function(){
		...
		$.each($(".mean-bar .progress-bar"), function(i, el){
			...
			$(el).parent().tooltip({ title: Math.round(width*100) + "% based on previous selections, " + Math.round((1-width)*100) + "% on variation" })
			$(el).html(Math.round(width*100) + "% based on previous selections");
		});
	}
...
$(document).on("ready", function(){
	...
	$(".bookmark-history-display-trigger").live("click", function(e){
		...
		if($(".bookmark-history-display").is(":hidden")){
			$("html, body").animate({ scrollTop: $(".bookmark-history-container").position().top }, "slow");
			$(".bookmark-history-display .bookmark").addClass("animated bounceIn");
			
			setTimeout(function(){
				$(".bookmark-history-display .bookmark").removeClass("animated bounceIn");
			}, 800);
